...
def _synthetic_screenshot(path: Path) -> None:
    ...
    _dark_large_digit(image, left, top, 2, 2, 4)
...
def _large_digit(image, left, top, row, col, digit, color=(80, 80, 80)) -> None:
...
def _dark_large_digit(image, left, top, row, col, digit) -> None:
    cell_left = left + col * 100
    cell_top = top + row * 100
    cv2.rectangle(image, (cell_left + 5, cell_top + 5), (cell_left + 95, cell_top + 95), (55, 55, 55), -1)
    _large_digit(image, left, top, row, col, digit, color=(245, 245, 245))
